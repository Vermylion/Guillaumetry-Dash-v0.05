...
def load_level(win_w, win_h):
    global level, player, player_rect_bottom, player_rect_right, player_rect_top, level_height, level_length, list_of_collisions, list_of_collisions_enemy,  list_of_collisions_jump_pad, list_of_collisions_jump_orb
    
    #Import level, player and collisions from
    level, player, player_rect_bottom, player_rect_right, player_rect_top, level_height, level_length, list_of_collisions, list_of_collisions_enemy,  list_of_collisions_jump_pad, list_of_collisions_jump_orb = LevelCreate.load_level(win_w, win_h)
    player.convert_alpha()
    level_length = 100 * level_length + 1000 

    #Give Physics all current collision lists
    Physics.set_collisions_lists(list_of_collisions, list_of_collisions_enemy, list_of_collisions_jump_pad, list_of_collisions_jump_orb, level_height)

#Main function to run all code based on the level
def play_level(screen, event, delta_time, win_w, win_h):
    global not_touching_ground, orb_save, gravity_offset, player_pos, level, player, level_move_speed, level_height, level_length, key_down
    ...
    #if keys are pressed (up, space, mouse button left), tell Physics that we are jumping
    if key_down:
        if key_type == pygame.K_UP:
    ...
    gravity_offset, player_pos, not_touching_ground, orb_save, reset = Physics.cube_physics(delta_time, level_move_speed, orb_save)
    ...
    if gravity_offset < -320:
        ...
        screen.blit(player, (300, (win_h - 385) + player_pos))
    else:
